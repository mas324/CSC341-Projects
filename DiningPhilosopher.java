...
public class DiningPhilosopher {
    ...
    static enum pState {STARTING, THINKING, HUNGRY, LCHOP, RCHOP, EATING, 
                        SLEEPING, NONE}
...
************************/
    ...
    class Board {
        ...
        public void setPhilosopher(int who, pState action){
            if(who < 0 || who > NUM_PHILOSOPHERS) System.exit(1);
            int row=0, col=0;
            int c=' ', id='0';
            // where is the Philosopher in the board (row, col)
            switch(who) {
                case 0: row = ph0r; col = ph0c; id='0';break;
                case 1: row = ph1r; col = ph1c; id='1';break;
                case 2: row = ph2r; col = ph2c; id='2';break;
                default: row = ph3r; col = ph3c; id='3';break;
            ...
            switch(action) {
                case STARTING: c = id; 
                    break;
                case THINKING: c = 192+((int) (Math.random() * 100)%6); 
                ...
                case RCHOP:    c = '>';
                    break;
                case EATING: c = 200+((int) (Math.random() * 100)%4);
                    break; // 200-203 Latin E with grave, acute, circumflex, or diaerese;
                case SLEEPING: int tmp = ((int) (Math.random() * 100)%4); // sequence through 4 values 
                            c = (tmp==0)?'s':(tmp==1)?'S':167;//(tmp==2)?138:167; // 's', 'S', S with caron or Section sign 
                    break;
                default:    c = id;   // 186; // Ordinal indicator or could be '.';
            }
...
************************/
        public void getChopstick(int chop, int who) {
            if(chop < 0 || chop > NUM_PHILOSOPHERS) System.exit(1);
            switch(who) {
                case 0:   
    ...
    public static class Philosopher extends Thread  {
       ...
       private void sleep() {
        ...
        private void eat() {
            ...
            try {
                /* break DEADLOCK by using one of the asymmetric solutions */
                /*
                    One philosopher picks up chopstick in reverse order.  
                    E.g. One philosopher picks left chopstick first (then trys for right)

                    or

                    Odd-numbered philosopher picks up first the left chopstick 
